...
```{r}
#| label: setup
# defines name of chunk, allowing for easier access

library(tidyverse)
theme_set(theme_bw(18)) # applies theme to whole document
...
```{r}
# beta0
storm_summ$coefficients[1,1] # about 3.9

# beta1
storm_summ$coefficients[2,1] # about 2.2

# sigma
storm_summ$sigma # about 2.3
```
...
$\beta_0$: With 0 inches of weekly precipitation, the estimated gage height would be 3.9 feet.

$\beta_1$: For every additional inch of weekly precipitation, the estimated increase in gage height is 2.2 feet.
...
y_hat <- 3.9 + 2.2 * x # our line; vector of estimated gage heights
...
R2 <- (SST - SSE) / SST # same as the number outputted in our model!

```

**Q4: What's the difference between** $y$**,** $\bar y$**, and** $\hat y$**?**

$y$: Observed gage height.

$\bar y$: Mean gage height overall.

$\hat y$: Expected gage height (given precipitation)

**Q5: What does the** $R^2$ **value tell you about the quality of the model (in plain language)?**

The $R^2$ value of 0.55 tells you that we have removed about half of the variance. Having precipitation reduces the error of our prediction by 55%.
...
# Simulating data with these parameters
beta_0 <- 2
beta_1 <- 4
sigma <- 0.5
...
mu <- beta_0 + beta_1 * x
y <- rnorm(n = 100, mean = mu, sd = sigma) # creating a normally distributed y variable
...
## Rockfish survey gaps (Midterm Review?)
