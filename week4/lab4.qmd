...
restoration <- read_csv(here("week4/data/eelgrass.csv"),
...
The test statistic is a difference of proportions.
...
```{r}
proportions <- restoration %>% 
  group_by(treatment) %>% 
  summarize(prop_success = sum(success)/n())

diff <- proportions$prop_success[2] - proportions$prop_success[1]
```
...
We want to shuffle by the treatment column.
...
one_permutation <- restoration %>% 
  mutate(treatment = sample(treatment, # reassigning the treatment column to random treatments
...
null_distribution <- map_dbl(1:1000, permute) # 1000 times, run the permutation
# w/out (), it is the function itself (not the output of function + argument)
...
ggplot() +
  ...
  geom_vline(xintercept = diff,
             color = "red",
             linewidth = 1) +
  labs(title = "Eel Grass Restoration",
       subtitle = "Null Distribution",
       x = "Difference of Proportions",
       y = "Frequency")
...
p_val <- sum(null_distribution >= diff)/length(null_distribution)
